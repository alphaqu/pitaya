...
pub struct Painter {
	...
	textures: FxHashMap<egui::TextureId, Rc<SrgbTexture2d>>,
...
impl Painter {
	...
	pub fn paint_and_update_textures<T: glium::Surface>(
		...
		for (id, image_delta) in &textures_delta.set {
			self.set_texture(facade, *id, image_delta);
		}

		self.paint_primitives(facade, backend, target, pixels_per_point, clipped_primitives);
	...
	pub fn paint_primitives<T: glium::Surface>(
		...
		} in clipped_primitives
		{
			match primitive {
				Primitive::Mesh(mesh) => {
					self.paint_mesh(target, facade, backend, pixels_per_point, clip_rect, mesh);
				}
	...
	fn paint_mesh<T: glium::Surface>(
		...
		let index_buffer =
			glium::IndexBuffer::new(facade, PrimitiveType::TrianglesList, &mesh.indices).unwrap();

		let (width_in_pixels, height_in_pixels) = backend.get_framebuffer_dimensions();

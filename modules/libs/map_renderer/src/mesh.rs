...
impl MeshBuilder {
	pub fn compile<S: Styler>(styler: &S, mut data: TileData, zoom: u8, scale: f32) -> MeshBuilder {
		let mut counter = MeshLengthCounter {
			length: 0
		};
	...
	pub fn build(self, ctx: &Rc<Context>) -> Mesh {
		Mesh {
			...
			indices: IndexBuffer::immutable(ctx, PrimitiveType::TrianglesList, &self.compile.indices).unwrap(),
	...
	pub fn update<S: Styler>(&mut self, styler: &S, zoom: f32, scale: f32) {
		...
		if change_ratio > FULL_WRITE_THRESHOLD {
			...
			for range in update.changes {
				let slice = self.vertices.slice_mut(range.clone()).expect("Slice does not exist for this range");
...
impl<'b> StyleHandler for MeshUpdate<'b> {
	fn submit<'a, S: Style>(&'a mut self, input: impl Into<S::Input<'a>>, mut style: S) {
		...
		self.pos += 1;
		let old_styler: &mut S = alloc.old_style.downcast_mut::<S>().expect("Stylers changed with the same tile data. This is very wrong.");

    ...
    pub fn tick(&mut self, sys: &System) {
        if !self.dropped {
            ...
            if !input.pointer.primary_down() {
                ...
                self.placements.drain_filter(|_, placement| {
                    placement.get_animation(sys).get_value() == 0.0
                });
            }
        }
    }
...
impl Placement {
    pub fn new(from: Rect,rect: Rect, hovered: bool, location: impl Into<NewAppLocation>) -> Placement {
        Placement {
            from,
            rect,
            hovered,

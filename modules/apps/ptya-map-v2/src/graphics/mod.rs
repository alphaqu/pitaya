...
use crate::{draw_debug, MapStyler, test_draw_tile, TilePosition};
...
pub struct MapGraphics {
    ...
    tiles: FxHashMap<TilePosition, Mesh>,
}
        ...
        if !self.drawn_tiles.contains(&pos) {
            ...
            if let Some(mesh) = self.tiles.get_mut(&pos) {
                ...
                let cull = {
                    ...
                    mesh.update(styler, viewport.zoom as f32 - 1.0, (1.0 / tile.size().y()) as f32);

...
use map_renderer::data::{FeatureData, LayerData, TileData};
use map_renderer::geometry::fill::{MultiPolygonGeometry, PolygonGeometry};
use map_renderer::geometry::path::{MultiPathGeometry, PathGeometry};
...
impl Geometry {
		...
		match ty {
			...
			GeomType::LineString => {
				...
				while let Some(Command::MoveTo(values)) = deque.pop_front() {
					...
					paths.push(PathGeometry { points });
				}
			...
			GeomType::Polygon => {
				...
				Some(Geometry::Polygon(MultiPolygonGeometry { polygons }))
			}
			GeomType::Unknown => {
				panic!("unknwon")
			}
	...
	pub fn build(self) -> Option<GeometryData> {
		match self {
			Geometry::Point { .. } => None,
			Geometry::Line(path) => Some(GeometryData::Path(path)),
...
impl Value {
	...
	fn build(self) -> map_renderer::data::Value {
		match self {
			Value::String(v) => map_renderer::data::Value::String(v),
			Value::F32(v) => map_renderer::data::Value::F32(v),
			Value::F64(v) => map_renderer::data::Value::F64(v),
			Value::I64(v) => map_renderer::data::Value::I64(v),
			Value::U64(v) => map_renderer::data::Value::U64(v),
			Value::Bool(v) => map_renderer::data::Value::Bool(v),
		}
	}
}
